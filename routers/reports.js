...
module.exports = server => {
    ...
    server.post('/adminreporttopfood', async (req, res, next) => {
        try {
            let sql = "select ot.full_food_name food_name, count(*) as count,sum(fd.cost) cost, sum(fd.price) total from food fd , orders o , orderdetails ot where fd.id = ot.foodId and ot.orderId = o.id and o.statusId = 2 and date(o.order_datetime) between date('" + req.body.dt + "') and date('" + req.body.edt + "')  group by  fd.id order by  count(*) desc ";
            const r = await sequelize.query(sql, { type: sequelize.QueryTypes.SELECT }).then(r => {
                res.send(r);
                next();
            })
    ...
    server.post('/adminreportbyuser', async (req, res, next) => {
        try {
            let sql = "select  u.username, count(*) count,sum(ot.total) as total from orders o, users u , orderdetails ot where o.userId = u.id and o.id = ot.orderId and date(o.order_datetime) between date('" + req.body.dt + "') and date('" + req.body.edt + "') and o.statusId=2 group by u.id  order by  count(*) desc ";
            const r = await sequelize.query(sql, { type: sequelize.QueryTypes.SELECT }).then(r => {
                res.send(r);
                next();
            })
